const Videos = require("../database/database");
...
router.post("/add", upload.single("video"), async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Erreur lors de la création d'un vidéos" });
  }
});
...
router.get("/all_videos", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Erreur lors de la génération du liste des vidéos" });
  }
});
...
router.put("edit/:id", upload.single("video"), async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erreur lors de la mise à jour des vidéos" });
  }
});
...
router.delete("/delete/:id", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erreur lors de la suppréssion du vidéos" });
  }
});
